...
struct Sphere {
    ...
    double radius;
    Vec3 albedo; 
    bool is_metal; // NEW: Toggle between Matte Stone and Shiny Metal
};
...
int main() {
    ...
    std::cout << "Enter brightness/lean (1.0 standard): ";
    double lean; std::cin >> lean;

    std::vector<Sphere> world;
    // The "Mirror" Metal Sphere (Shiny Silver)
    world.push_back({Vec3(0, 0, -1), 0.5, Vec3(0.8, 0.8, 0.8), true}); 
    // The "Matte" Stone Ground (Greenish)
    world.push_back({Vec3(0, -100.5, -1), 100.0, Vec3(0.3, 0.6, 0.3), false}); 
    ...
    for (int j = cam.i_h - 1; j >= 0; --j) {
        ...
        for (int i = 0; i < cam.i_w; ++i) {
            ...
            for (int s = 0; s < 50; ++s) {
                ...
                col += ray_color(cam.get_ray(u, v), world, 10, lean);
            }
