...
Vec3 ray_color(const Ray &r, const std::vector<Sphere> &spheres, const std::vector<Triangle> &tris, int depth)
{
    ...
    {
        Vec3 tri_n;
        double t = hit_triangle_obj(tri, r, tri_n);
        if (t > 0.001 && t < closest_t)
        {
            hit = true;
            closest_t = t;
    ...
    if (hit)
    {
        ...
        return (metal ? 0.9 : 0.5) * albedo * ray_color(scattered, spheres, tris, depth - 1);
    }
...
int main()
{
    ...
    // 1. Metal Floor
    spheres.push_back({Vec3(0, -100.5, -1), 100.0, Vec3(0.8, 0.8, 0.8), true, 0.05});
    ...
    // 3. Rotated Cube (Foreground)
    double angle = 35.0;
    Vec3 offset(0.3, 0.0, -1.2); // Cube is closer and slightly right
    Vec3 col(0.2, 0.4, 0.8);
    ...
    for (auto &p : pts)
    {
        p = rotate_y(p, angle) + offset;
    }

    // Faces (Order matters for "Outward" normals)
    triangles.push_back({pts[0], pts[1], pts[2], col, false, 0.0});
    triangles.push_back({pts[0], pts[2], pts[3], col, false, 0.0}); // Front
    triangles.push_back({pts[5], pts[4], pts[7], col, false, 0.0});
    triangles.push_back({pts[5], pts[7], pts[6], col, false, 0.0}); // Back
    triangles.push_back({pts[4], pts[0], pts[3], col, false, 0.0});
    triangles.push_back({pts[4], pts[3], pts[7], col, false, 0.0}); // Left
    triangles.push_back({pts[1], pts[5], pts[6], col, false, 0.0});
    triangles.push_back({pts[1], pts[6], pts[2], col, false, 0.0}); // Right
    triangles.push_back({pts[3], pts[2], pts[6], col, false, 0.0});
    triangles.push_back({pts[3], pts[6], pts[7], col, false, 0.0}); // Top
    triangles.push_back({pts[4], pts[5], pts[1], col, false, 0.0});
    triangles.push_back({pts[4], pts[1], pts[0], col, false, 0.0}); // Bottom
    ...
    for (int j = cam.i_h - 1; j >= 0; --j)
    {
        ...
        for (int i = 0; i < cam.i_w; ++i)
        {
            ...
            for (int s = 0; s < 100; ++s)
            {
                ...
                color += ray_color(cam.get_ray(u, v), spheres, triangles, 10);
            }
            double sc = 1.0 / 100.0;
